...
/// Prefer `~/.kami/bin/grok` which always points to the latest version.
...
/// Delete `~/.kami/models_cache.json` after a successful update.
///
...
/// Remove the stale `grok-pager` symlink/binary from `~/.kami/bin/` left by
...
/// config and writes to `~/.kami/bin/`, so callers must isolate
/// `GROK_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.kami/downloads/`, not yet
...
///
/// When both paths share a grandparent (e.g. `~/.kami/bin/grok` and
/// `~/.kami/downloads/grok-0.1.203-linux-x86_64`), returns a relative path
...
///
/// Relative symlinks survive Docker bind-mounts where `~/.kami/` is mapped
...
/// bind-mounts of `~/.kami/`). Windows: [`windows_replace_exe`].
///
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // through it (e.g. /usr/local/bin/grok -> ~/.kami/downloads/grok-latest)
    ...
    // ~/.kami/downloads/ (legacy layout — skips the grok-latest indirection).
    ...
    for name in ["grok", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".kami/downloads/") && !target_str.ends_with("grok-latest") {
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.kami/bin/
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        let dest_181 = std::path::Path::new("/home/u/.kami/downloads/grok-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.kami/downloads/grok-0.1.182-linux-x86_64");
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.kami/downloads").into(),
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        ...
        let target = std::path::Path::new("/home/alice/.kami/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.kami/bin/grok");
    ...
    fn test_relative_symlink_target_same_dir() {
        ...
        let target = std::path::Path::new("/home/alice/.kami/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.kami/downloads/grok-latest");
        let result = relative_symlink_target(target, link);
    ...
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/grok -> /home/alice/.kami/downloads/grok-0.1.203
        ...
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.kami/downloads/grok-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.kami/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".kami");
