...
/// Assert the active `~/.kami/bin/grok` resolves to the expected versioned
...
// installers that actually maintain the managed `~/.kami/bin/grok` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.kami/downloads cleanup), the probe must report None — otherwise
