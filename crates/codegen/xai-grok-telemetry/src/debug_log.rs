...
//!   firehose to `~/.kami/debug/<session_id>.txt` (one file per session), with a
...
pub(crate) enum DebugTarget {
    /// `GROK_DEBUG_LOG=1` → route per session into `<dir>` (`~/.kami/debug`).
...
/// per session into `~/.kami/debug`, an explicit path writes a single file.
///
...
/// `~/.kami/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
