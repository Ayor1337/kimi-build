...
Sample hooks for Grok. Copy to `~/.kami/hooks/` to enable globally, or to `<project>/.kami/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.kami/hooks/bin
cp examples/hooks/safe-shell.json ~/.kami/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.kami/hooks/bin/
chmod +x ~/.kami/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.kami/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.kami/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.kami/hooks/bin/
chmod +x ~/.kami/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.kami/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.kami/hooks/bin
cp examples/hooks/session-log.json ~/.kami/hooks/
cp examples/hooks/bin/session-log.sh ~/.kami/hooks/bin/
chmod +x ~/.kami/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.kami/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.kami/hooks/bin
cp examples/hooks/tool-logger.json ~/.kami/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.kami/hooks/bin/
chmod +x ~/.kami/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.kami/hooks/`. The hook stops running on the next session.