...
/// User-level vs project-level config files (`~/.kami` vs `{cwd}/.kami`).
...
/// Union bundled personas with local `~/.kami/personas` and `{cwd}/.kami/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".kami").join("personas")),
...
/// Shared guard: canonical path under `~/.kami/{subdir}` or `{cwd}/.kami/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let project_suffix = std::path::Path::new(".kami").join(subdir);
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".kami").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".kami").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.kami/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".kami").join("personas").join("gone.toml");
