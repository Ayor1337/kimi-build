...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    ...
    // or a project-scoped `{cwd}/.kami/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".kami" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    ...
    // Project hooks
    if source_dir.ends_with("/.kami/hooks") || source_dir.contains("/.kami/hooks/") {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.kami/plugins/<name>/` must label as
        ...
        let (label, is_custom) = derive_source_label("/repo/work/.kami/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.kami/installed-plugins/vendor-abc123/skills");
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.kami/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
