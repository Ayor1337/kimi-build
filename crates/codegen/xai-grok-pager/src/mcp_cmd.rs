...
Examples:
  ...
  # Add to the project config (./.kami/config.toml) instead of ~/.kami/config.toml
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.kami/config.toml`, available in all your projects
    User,
    /// `./.kami/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.kami/config.toml) or project (./.kami/config.toml)
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        ...
        let project = PathBuf::from("/repo/.kami/config.toml");
