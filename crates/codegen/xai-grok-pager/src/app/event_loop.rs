...
pub(crate) async fn run(
    ...
    // Apply initial config (may come from existing ~/.kami/pager.toml).
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.kami/config.toml [ui]`
                // rather than `~/.kami/pager.toml`. The watcher only knows
