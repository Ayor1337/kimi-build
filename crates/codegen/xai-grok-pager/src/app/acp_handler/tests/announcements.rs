#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.kami` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.kami` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.kami` reads).
    #[test]
