...
See ~/.kami/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `~/.kami/config.toml` or when the `GROK_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.kami/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.kami/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.kami/logs/sampling.jsonl.
