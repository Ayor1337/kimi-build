...
///
/// Both variants write to `~/.kami/config.toml` and route ACP
...
pub enum Effect {
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.kami/config.toml`.
    ...
    /// Persist a typed setting to `~/.kami/config.toml`. On failure,
    ...
    /// registry (`~/.kami/active_sessions.json`).
