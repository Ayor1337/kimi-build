...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  kami --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  kami --resume {}", info.session_id);
    }
}
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - kami"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "kami");
        assert_eq!(terminal_title_string(""), "kami");
        assert_eq!(terminal_title_string("My chat"), "My chat - kami");
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  kami --resume sess-abc\n"));
    }
    #[test]
