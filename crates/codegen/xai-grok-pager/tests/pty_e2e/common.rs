...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let grok_home = content.home().join(".kami");
...
/// Seed `~/.kami/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{GROK_HOME|HOME}/.kami/config.toml` location
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let grok_home = content.home().join(".kami");
    std::fs::create_dir_all(&grok_home).expect("create .grok");
    // Minimal opt-in only — matches load_config's `{GROK_HOME|HOME}/.kami/config.toml`.
