...
Kimi Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.kami/hooks/`
- **Project hooks** — from `.kami/hooks/` in your repo
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.kami/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `kami plugin marketplace add <source>`)
...
Sources land in `~/.kami/config.toml`:
...
Or in `~/.kami/settings.json` / `~/.claude/settings.json`:
