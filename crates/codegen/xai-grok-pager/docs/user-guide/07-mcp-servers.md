...
MCP servers are configured in `~/.kami/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.kami/config.toml`) **or repo-level**
>   (`.kami/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.kami/config.toml` >
...
kami mcp list
kami mcp list --json          # Machine-readable output
...
kami mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
kami mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
kami mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
kami mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
kami mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
kami mcp remove github

# Diagnose a server's configuration and connectivity
kami mcp doctor               # Check every configured server
kami mcp doctor github        # Check one server
kami mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `kami mcp add` writes to `~/.kami/config.toml` (`--scope user`). Use `--scope project` to write to `.kami/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `kami mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`kami mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.kami/config.toml` in your repository:
...
Grok walks from the current directory up to the git repo root, loading `.kami/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.kami/config.toml` | All projects | Lowest |
| `<repo-root>/.kami/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.kami/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.kami/config.toml`, `.kami/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.kami/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `kami inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).
...
For stdio servers, Grok captures the process's standard error to `~/.kami/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.kami/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `kami inspect` to see all loaded MCP servers and their sources:

```bash
kami inspect          # Human-readable
kami inspect --json   # Machine-readable
```
