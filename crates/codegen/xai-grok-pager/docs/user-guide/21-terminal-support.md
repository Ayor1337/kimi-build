...
**Optional workaround for live clipboard**: Use `kami wrap ssh` instead of plain `ssh` (for example, `kami wrap ssh user@host`). It runs the command in a local PTY that intercepts OSC 52 sequences, including tmux-wrapped ones, and writes their contents to your local clipboard. The same command wraps anything else whose clipboard can't reach you — for example `kami wrap docker exec -it <container> bash` or `kami wrap kubectl exec -it <pod> -- bash`.
