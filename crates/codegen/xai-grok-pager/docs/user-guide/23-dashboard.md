...
- **`kami dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.kami/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Kimi Build · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Kimi Build's sibling views (
`tasks_pane`):
...
process. The plan's Phase 4 ("supervisor / `kami --bg`") would list
