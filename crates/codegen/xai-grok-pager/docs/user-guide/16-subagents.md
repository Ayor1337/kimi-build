...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.kami/agents/` or `~/.kami/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.kami/personas/`; applied during subagent resolution |
...
Kimi Build discovers file-based personas from these locations, in priority order:

- `.kami/personas/*.toml` (project)
- `~/.kami/personas/*.toml` (user)
...
> **Note:** Kimi Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Kimi Build resolves the effective model and reasoning effort in this order, highest priority first:
...
Kimi Build manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
# instructions_file = ".kami/personas/concise.md"  # or load from a file
```

Kimi Build also discovers roles from `.kami/roles/*.toml` and personas from `.kami/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Kimi Build shows running and finished work in side panes on the agent screen:
