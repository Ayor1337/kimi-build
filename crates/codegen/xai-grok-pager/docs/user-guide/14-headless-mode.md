...
kami -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
kami -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
kami -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
kami -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
kami -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
kami -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
kami -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
kami -p "Build the project" --allow "Bash"
```
...
By default, each `kami -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
kami -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
kami -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
kami -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
kami -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
kami -p "What's the secret number?" --resume abc123
```
...
```bash
kami -p "Continue where we left off" -c
```
...
```bash
kami -p "Hello" --output-format json | jq -r '.sessionId'
```
...
kami -p "Generate a README" > README.md

# Parse JSON output with jq
kami -p "List files" --output-format json | jq -r '.text'
```
...
kami -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
kami --prompt-file ./prompt.txt
```
...
```bash
kami -p "Review changes for bugs and security issues." \
...
```bash
kami -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
kami -p "Format all files" --yolo

# Run tests and fix failures
kami -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
kami -p "Run the test suite" --yolo
```
...
- **`kami login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`kami login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `kami -p "..." --yolo --rules "Never delete files"`.
...
Grok stores data in `~/.kami` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.kami` read-only:
...
- Resume: `kami -p "continue" --resume "<id>"` or `kami -p "continue" --continue`
