...
pub enum ImportScope {
    /// User-level: writes to `~/.kami/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.kami/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.kami/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.kami/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.kami/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.kami/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.kami/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.kami/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.kami/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.kami/hooks/imported-from-claude.json.
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".kami").join("config.toml");
        ...
        let hooks_dir = project_root.join(".kami").join("hooks");
...
mod tests {
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.kami) don't interfere.
...
"#;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.kami/config.toml` may legitimately have the marker
