...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.kami/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.kami` (the writer may also be this process;
...
/// Watches `~/.kami/` for `auth.json`, `config.toml`, and `models_cache.json`
...
/// `<cwd>/.kami/` (catches `<cwd>/.kami/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.kami/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.kami/config.toml` so the loop above
        // watches `<cwd>/.kami/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.kami/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.kami/config.toml` trigger a
    ...
    /// `<cwd>/.kami/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.kami/` does not yet
/// exist at session-open time, the `.grok/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.kami/` followed by a
/// write to `<cwd>/.kami/config.toml` will NOT be observed — the
...
/// expected for a freshly-opened session whose `<cwd>/.kami/` hasn't
...
/// Watches skill directories (`~/.kami/skills/`, `<repo>/.kami/skills/`, etc.)
...
/// recursively: `grok_home` (`~/.kami`, or `$GROK_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.kami` alone has `worktrees/`,
...
const HOME_VENDOR_DIRS: &[&str] = &[".kami", ".agents", ".claude", ".cursor"];
...
/// `~/.kami` these exist at startup; later creation is caught on restart).
...
mod tests {
    ...
    fn is_global_config_dir_matches_only_grok_home_and_vendor_dirs() {
        ...
        assert!(!g(&home.join("repo").join(".kami")));
    }

    /// Regression for the ~/.kami inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.kami/worktrees/`) must not
    ...
    /// A write to `<cwd>/.kami/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".kami");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.kami/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.kami/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".kami");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
