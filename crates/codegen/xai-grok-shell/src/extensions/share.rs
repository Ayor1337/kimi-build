...
mod tests {
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // We must explicitly set oidc_issuer to a first-party Kimi issuer.
        // Only OIDC tokens against https://auth.kimi.com (or the local-dev equivalent)
        ...
        let auth = GrokAuth {
            ...
            oidc_issuer: Some("https://auth.kimi.com".to_string()),
            key: "test-key".into(),
