...
//!   under `~/.kami/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.kami/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.kami/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.kami/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.kami/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.kami/`.
...
/// `CleanAll` mode. Placed under `~/.kami/worktree_pool/<instance_id>/<uuid>/`.
