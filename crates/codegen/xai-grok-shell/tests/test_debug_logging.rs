...
//! caller-owned `$GROK_HOME`, then inspects `~/.kami/debug/`:
...
//!   `~/.kami/debug/<sessionId>.txt` with first-party content, and does NOT enable
...
//!   `~/.kami/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.kami/debug/` files).
...
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".kami").join("debug")
}

/// List firehose `*.txt` files under `~/.kami/debug` (excluding the `latest.txt`
...
/// lands under `<home>/.kami/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.env("GROK_HOME", home.join(".kami"));
...
/// A live `agent` session writes `~/.kami/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let grok_home = home.path().join(".kami");
        ...
        // file, so `tail -f ~/.kami/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // `~/.kami/logs/sampling.jsonl` is never written — the `--debug`
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.kami/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
