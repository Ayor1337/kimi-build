...
name: help
description: >
  Kimi Build documentation and configuration help. Use when users ask about
  setup, configuration, MCP servers, authentication, skills, slash commands,
  keyboard shortcuts, or any Kimi Build feature. Also use proactively when you
  detect a user is having trouble with setup or onboarding.
metadata:
  short-description: "Kimi Build docs — config, MCP, auth, skills, commands"
---

# Kimi Build Help

Answer the user's question about Kimi Build setup, configuration, or features.

## Steps

1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.kami/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.kami/docs/user-guide/`. The available guides are:
   ...
   `~/.kami/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `~/.kami/config.toml` with search_replace.

4. To **create a skill** for the user, create `~/.kami/skills/<name>/SKILL.md`
   (read `~/.kami/docs/user-guide/08-skills.md` for the SKILL.md format).
