...
//! (release builds, self-extracted to `~/.kami/vendor/<name>-<ver>-<target>`) →
//! `~/.kami/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.kami/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.kami/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.kami/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.kami/vendor` and the extracted `bfs` runs.
