...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.kami/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.kami/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.kami/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.kami/ or ~/.claude/.
    User { path: PathBuf },
