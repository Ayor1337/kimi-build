...
/// User-facing label for the user grok directory (``~/.kami`` or ``$GROK_HOME``).
///
...
/// User-facing path under [`grok_home()`], e.g. ``~/.kami/config.toml``.
...
/// True when `path` is under user [`grok_home()`] (not project `{cwd}/.kami`).
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        ...
        assert_eq!(display_grok_home_prefix(), "~/.kami");
    }
    ...
    fn display_user_grok_path_joins_relative() {
        ...
        assert!(path.contains(".kami") || path.contains("$GROK_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.kami/memory/MEMORY.md");
