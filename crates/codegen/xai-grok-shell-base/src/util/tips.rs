...
//! persisted to `~/.kami/tip_cursor.json`.
...
/// Load the cursor from `~/.kami/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.kami/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.kami/tip_cursor.json`, so every session sees the next tip
