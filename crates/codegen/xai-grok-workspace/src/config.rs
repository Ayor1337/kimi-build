...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.kami/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.kami/hooks/`).
    ...
    /// `<cwd>/.kami/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.kami/hooks/`).
    Directory(PathBuf),
}
