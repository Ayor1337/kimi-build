...
/// Directory (under `<cwd>/.kami`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.kami/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
