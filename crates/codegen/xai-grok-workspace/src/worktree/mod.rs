...
/// Returns `~/.kami/worktrees/<repo_slug>` for the given git root.
///
...
/// Resolves the worktree base directory (`~/.kami/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.kami/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.kami/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.kami/worktrees/`.
...
///
/// When the source path is already inside `~/.kami/worktrees/<repo>/`, the
...
/// Scan `~/.kami/worktrees/<repo_name>/` for subdirectories not tracked
