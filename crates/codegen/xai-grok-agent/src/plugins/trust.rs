...
//!
//! **Trust storage**: `~/.kami/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.kami/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.kami/trusted-plugins` does not exist, returns an empty store.
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.kami/trusted-plugins`.
    ...
    /// rewrites `~/.kami/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.kami/trusted-plugins`.
